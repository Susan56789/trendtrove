            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                <div class="image-container relative flex items-end overflow-hidden rounded-xl">
                  <img :src="product.image" :alt="product.name" class="mx-auto w-full h-full object-cover mb-2" />
                ...
                <h2 class="text-slate-700 text-xs">{{ product.name }}</h2>
                <p class="mt-1 text-xxs text-slate-400">{{ product.description }}</p>
                <div class="mt-2 flex items-end justify-between">
                  <p class="text-gray-500 line-through text-xxs">KES. {{ formatNumber(product.price) }}</p>
