    ...
    <article class="rounded-xl bg-yellow-50 p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300">
      <div class="mt-1 p-2">
        ...
        <div class="mt-3 flex items-end justify-between">
          <p v-if="product.discountedPrice" class="text-gray-500 line-through">KES. {{ formatNumber(product.price) }}</p>
          <p v-if="product.discountedPrice" class="text-green-500 font-semibold">KES. {{ formatNumber(product.discountedPrice) }}</p>
          ...
          <button @click="addToWishlist" class="bg-green-500 text-white px-2 py-1.5 rounded-lg duration-100 hover:bg-green-700">
            Add to Wishlist
          </button>
        </div>
      </div>
